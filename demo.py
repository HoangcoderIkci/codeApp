...
def find_systems_ort_norm_basic():
    matrix = sympy.Matrix(init_matrix())
    x= sympy.symbols('x')
    row,col = matrix.shape
    t = min(row,col)
    print(num_cols, num_rows,t)
    for i in range(t):
        matrix[i,i] -=x
    print("matrix: ", matrix)
    det = matrix.det()
    ...
    print(type(det))
    print("det: ",det)
    factored_expr = sympy.factor(det)
    terms = list(factored_expr.args)
    #sorted_terms = sorted(terms, key=lambda term: term.degree(x))

# Tạo danh sách các biểu thức từ các thuật ngữ đã sắp xếp
    expr_list = [sympy.Mul(term) for term in terms]
# Lấy danh sách các thuật ngữ trong biểu thức đã phân tích thành nhân tử
    print("terms:",expr_list)
...
find_ort_norm_basis_button = tk.Button(root, text="find_basis", command=find_systems_ort_norm_basic)
find_ort_norm_basis_button.grid(row=4, column=2)
